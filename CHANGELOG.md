...
- Tooltip event listener memory leak: mousedown/keydown handlers now properly cleaned up on dismiss
- Type safety: metadata payload typed as `InitializrMetadata` instead of `unknown`
- External link error handling: added try/catch for invalid URLs in `openExternalLink`
- History date format: month, day, hour now consistently zero-padded
